...
RUN --mount=type=cache,target=/go/pkg/mod/ \
    ...
    CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -a -trimpath -ldflags "-X main.version=${VERSION} -X main.commit=${COMMIT} -X main.date=${DATE}" -o /bin/flagd-proxy flagd-proxy/main.go

# Fail closed if the FIPS build settings did not reach the binary.
RUN go version -m /bin/flagd-proxy | grep -q 'GOFIPS140=v1\.0\.0' \
      || (echo "ERROR: /bin/flagd-proxy is not a FIPS 140-3 build" && exit 1) \
    && go version -m /bin/flagd-proxy | grep -E 'GOFIPS140=|-tags=fips140'
