...
```
  ...
  -t, --metrics-exporter string              Set the metrics exporter. Default(if unset) is Prometheus. Can be override to otel - OpenTelemetry metric exporter. Overriding to otel require otelCollectorURI to be present
      --ofrep-compression                    Gzip OFREP evaluation responses when the client sends Accept-Encoding: gzip. Disable when a proxy in front of flagd already compresses. Defaults to true. (default true)
      --ofrep-compression-min-size int       Smallest OFREP response body (bytes) that is gzipped. Below this, compression costs more CPU than the bytes it saves. Set to 0 to compress every response. Defaults to 1024. (default 1024)
