...
| `regex_match` (_available TBD_)    | Attribute matches the specified regular expression  | string                                       | Logic: `#!json { "regex_match" : [ "noreply@example.com", ".*@example.com"] }`<br>Result: `true`<br><br>Logic: `#!json { regex_match" : [ "noreply@example.com", ".*@test.com"] }`<br>Result: `false`<br>Additional documentation can be found [here](./custom-operations/string-comparison-operation.md). |
